...
fn test_ordering_sensitivity() {
    ...
    assert_ne!(
        root_12, root_21,
        "BMT root MUST be sensitive to log ordering"
    );
}
...
fn test_injection_resistance() {
    ...
    assert_ne!(
        base_root, injected_root,
        "BMT root MUST change if logs are injected"
    );
}
...
fn test_duplicate_symbols_uniqueness() {
    ...
    assert_ne!(
        bmt.root(),
        BehavioralMerkleTree::new(vec![BehavioralSymbol::new("Tf", 10)]).root()
    );
}
...
fn test_substitution_detectability() {
    ...
    assert_ne!(
        base_root, substituted_root,
        "BMT root MUST change on symbol substitution"
    );
}
