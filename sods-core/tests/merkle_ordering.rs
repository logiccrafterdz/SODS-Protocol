...
fn test_onchain_proof_has_is_left_path() {
    ...
    let proof = bmt
        .generate_onchain_proof(&matched, 1, 100, None, 0)
        .unwrap();

    // is_left_path must be populated
    assert!(
        !proof.is_left_path.is_empty(),
        "is_left_path should not be empty"
    );
...
fn test_directions_match_proof_struct() {
    ...
    assert_eq!(
        std_proof.directions, onchain_proof.is_left_path,
...
fn test_calldata_includes_is_left_path() {
    ...
    let proof = bmt
        .generate_onchain_proof(&matched, 11155111, 100, None, 0)
        .unwrap();
    ...
    assert!(
        calldata.len() > 256,
        "Calldata should include is_left_path array"
    );
    assert_eq!(
        calldata.len() % 32,
        0,
