use prometheus::{Registry, IntGauge, Counter, Histogram, HistogramOpts, TextEncoder, Opts, Gauge, Encoder};
...
impl AgentMetrics {
    pub fn new() -> Result<Self, prometheus::Error> {
        ...
        let verification_duration_seconds = Histogram::with_opts(HistogramOpts::new("sods_verification_duration_seconds", "Time spent verifying blocks"))?;

        // ERC-8004
        let registry_registrations_total = Counter::new("sods_registry_registrations_total", "Total agent registrations")?;
        let registry_updates_total = Counter::new("sods_registry_updates_total", "Total agent metadata updates")?;
        let validation_requests_received_total = Counter::new("sods_validation_requests_received_total", "Total validation requests received")?;
        let validation_responses_submitted_total = Counter::new("sods_validation_responses_submitted_total", "Total validation responses submitted")?;
        let validation_success_rate = Gauge::new("sods_validation_success_rate", "Current validation success rate percentage")?;
        let feedback_received_total = Counter::new("sods_feedback_received_total", "Total reputation feedback entries received")?;
    ...
    pub async fn start_http_server(self: Arc<Self>, port: u16) {
        ...
        let app = app.route("/_metrics", get(|State(m): State<Arc<AgentMetrics>>| async move {
