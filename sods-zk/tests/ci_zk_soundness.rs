...
fn test_zk_soundness_non_existent_behavior() {
    ...
    match result {
        Ok(receipt) => {
            ...
            assert!(
                !valid,
                "ZK Soundness Violation: Proved non-existent behavior as true"
            );
        }
...
fn test_zk_soundness_substitution_attack() {
    ...
    if let Ok(receipt) = result {
        ...
        assert!(
            !valid,
            "ZK Soundness Violation: Substitution attack succeeded"
        );
    }
...
fn test_zk_soundness_reordering_attack() {
    ...
    if let Ok(receipt) = result {
        ...
        assert!(
            !valid,
            "ZK Soundness Violation: Out-of-order sequence proved true"
        );
    }
