...
async fn test_multi_chain_receipts_root_validation() {
    ...
    let chains = vec![
        ChainData {
            name: "Arbitrum One",
            rpc_url: env::var("ARB_RPC_URL")
                .unwrap_or_else(|_| "https://arb1.arbitrum.io/rpc".to_string()),
            block_number: 290000000,
            expected_receipts_root:
                "0x3938be9ae329068595aeb4573887c3315978f870956461a5256e9f5e33d0273a"
                    .parse()
                    .unwrap(),
        },
        ChainData {
            name: "Optimism",
            rpc_url: env::var("OP_RPC_URL")
                .unwrap_or_else(|_| "https://mainnet.optimism.io".to_string()),
            block_number: 120000000,
            expected_receipts_root:
                "0x7856345634563456345634563456345634563456345634563456345634563456"
                    .parse()
                    .unwrap(), // Placeholder
        },
        ChainData {
    ...
    for chain in chains {
        println!(
            "Testing receipts root on {}: block {}",
            chain.name, chain.block_number
        );
        ...
        match verifier
            .verify_symbol_in_block("Tf", chain.block_number)
            .await
        {
            Ok(res) => {
                ...
                assert!(
                    res.verification_mode == VerificationMode::Trustless
                        || res.verification_mode == VerificationMode::ZeroRpc
                );
            }
            Err(sods_verifier::error::SodsVerifierError::InvalidReceiptProof {
                computed,
                expected,
            }) => {
                panic!(
                    "Root mismatch on {}!\nComputed: {}\nExpected: {}",
                    chain.name, computed, expected
                );
            }
            Err(e) => {
