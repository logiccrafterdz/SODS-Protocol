...
async fn test_sepolia_integration_latest_block() {
    ...
    assert!(health, "Health check should pass for a valid Sepolia RPC URL");
    ...
    assert!(result.is_ok(), "Failed to query block 5000000: {:?}", result.err());
    ...
    assert!(future_result.is_err(), "Querying an unreachable future block should return an Error");
}
