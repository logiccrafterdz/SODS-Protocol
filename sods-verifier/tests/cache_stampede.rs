...
async fn test_extreme_cache_stampede_prevention_1000() {
    ...
    println!(
        "🔥 Starting extreme stress test: 1000 concurrent tasks for block {}...",
        block_num
    );
    ...
    assert_eq!(
        total_calls, 1,
        "Cache stampede! Expected 1 RPC call, but got {}. Concurrency protection failed.",
        total_calls
    );
...
async fn test_sequential_vs_concurrent_performance() {
    ...
    println!(
        "🚀 Concurrent burst (50 reqs): {:?} | Calls: {}",
        duration_c, calls_c
    );
    println!(
        "🏃 Sequential (50 reqs):      {:?} | (Hot Cache)",
        duration_s
    );

    assert_eq!(calls_c, 1, "Concurrent burst should only make 1 call");
    assert!(
        duration_s < duration_c,
        "Sequential hot cache should be faster than initial burst"
    );
}
