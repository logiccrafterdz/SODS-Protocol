...
async fn test_backoff_profiles_delays() {
    ...
    assert!(
        l2.delays()[0] > eth.delays()[0],
        "L2 should have longer initial delay"
    );
    assert!(
        l2.delays()[2] > eth.delays()[2],
        "L2 should have longer max delay"
    );
}
...
async fn test_total_exhaustion_error() {
    ...
    assert!(
        result.is_err(),
        "Should return error when all RPCs are dead"
    );
}
